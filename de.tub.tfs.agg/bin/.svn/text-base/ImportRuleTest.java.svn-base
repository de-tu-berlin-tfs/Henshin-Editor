 ...
 */
public class ImportRuleTest implements GraTraEventListener {

	public ImportRuleTest() {
...
//			graphGrammarTransform();
	...
	public static void main(String[] args) {
		ImportRuleTest importRuleTest = new ImportRuleTest();
		
		final GraGra gragraOut = load(args[0]);
		
		final List<File> list = new Vector<File>();		
		final File file = new File(args[1]);
		list.add(file);
		
		start(list, gragraOut);
...
//		Enumeration<MorphCompletionStrategy> strategies = CompletionStrategySelector.getStrategies();
		...
		// graphGrammar.save(fn);
		// System.out.println("Grammar "+graphGrammar.getName()+" saved in
		// "+fn);
		...
		for (int i = 0; i <= num; i++) {
			...
			// Set this.strategy of match completion
...
//					Step step = new Step();
	...
	private boolean generatePostApplicationConditionOfRule(GraGra gragra,
			Rule rule) {
		// Convert AGCs to Post Application Consistency Constraints of rule
	...
	public static void start(List<File> ruleFiles, GraGra gragraOut) //throws AggErrorException 
	{
		// TypeGraph im Output pruefen
	    gragraOut.setLevelOfTypeGraphCheck(TypeSet.DISABLED);
//	    gragraOut.getTypeSet().clearTypeGraph();

	    // Alle Rulefiles durchlaufen
	    for (File file : ruleFiles) {
//	    	logger.info("Loading rules from " + file.getName() + "...");
	    	System.out.println("Loading rules from " + file.getName() + "...");
	        // File laden
	        XMLHelper h = new XMLHelper();
	        if (!h.read_from_xml(file.getPath()))
	        	continue;
	        
	        GraGra gragraRule = new GraGra(false);
	        h.getTopObject(gragraRule);

	        // Typegraphen importieren, falls vorhanden
	        if (gragraRule.getTypeGraph() != null) {
//	        	logger.info("Importing Typegraph...");
	        	System.out.println("Importing Typegraph...");
	            gragraOut.setLevelOfTypeGraphCheck(TypeSet.DISABLED);
	            gragraOut.importTypeGraph(gragraRule.getTypeGraph(), true);

	            Collection<TypeError> errors =
	            			gragraOut.setLevelOfTypeGraphCheck(TypeSet.ENABLED);
	            if (errors.size() > 0) {
//	                 throw new AggErrorException("Imported typegraph could not be activated");
	            	System.out.println("Imported typegraph could not be activated");
	            }
	            else {
//	            	gragraOut.save("SAVED_TG_"+gragraOut.getFileName());
	            }
	       }

	       // Regeln durchlaufen
	       List<Rule> rules = gragraRule.getListOfRules();
//	       logger.info("Copying rules from " + file.getName() + "...");
	       System.out.println("Copying rules from " + file.getName() + "...");
	       for (Rule rule : rules) {
//	           logger.info("Adding rule '" + rule.getName() + "...");
	    	   System.out.println("Adding rule '" + rule.getName() + "...");
	           //Kopieren
	           try {
	                gragraOut.addImportRule(rule, true);
	           } catch (Exception e) {
//	                    throw new AggErrorException("Error adding rule: " +e.getMessage());
	    ...
	    Collection<TypeError> errors =
	        	gragraOut.setLevelOfTypeGraphCheck(TypeSet.ENABLED);
	    if (errors.size() > 0) {
//	       	throw new AggErrorException("Final typegraph could not be activated");
	    	System.out.println("Final typegraph could not be activated");
	    }
	    else {
//        	gragraOut.save("RULES_SAVED_TG_"+gragraOut.getFileName());
        }
	    
	    for (Graph g : gragraOut.getListOfGraphs()) {
	...
	public static GraGra load(String fName) {
		...
		if (fName.endsWith(".ggx")) {
			final XMLHelper h = new XMLHelper();
