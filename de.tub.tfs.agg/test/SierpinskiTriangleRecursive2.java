...
public class SierpinskiTriangleRecursive2 extends SierpinskiTriangleBasic {
	...
	public SierpinskiTriangleRecursive2(String filename) {
		this(filename, "1");
	}
	
	public SierpinskiTriangleRecursive2(String filename, String nn) {
		super(filename, nn);
	}
...
//					+" *** edges: "+gragra.getGraph().getArcsCount()
	...
	private Arc getOutEdge(GraphObject go, String edgeTypeName) {
		...
		while (outArcs.hasNext()) {
			...
			if (arc.getType().getName().equals(edgeTypeName)) {					
				return arc;
			}
		}
		return null;
	...
	private Arc getOutEdge(GraphObject src, String edgeTypeName, GraphObject tar) {
		...
		while (outArcs.hasNext()) {
			Arc arc = outArcs.next();
			if (arc.getType().getName().equals(edgeTypeName) && arc.getTarget() == tar) {					
				return arc;
			}
		}
		return null;
	...
	private boolean applyRule1(
		...
		for (int i=0; i<matchVecForR.size(); i++) {
			...
			try {							
				m.addMapping(n1ofR1, objs.get(0));
				m.addMapping(n2ofR1, objs.get(1));
				m.addMapping(n3ofR1, objs.get(2));
				m.addMapping(a1ofR1, objs.get(3));
				m.addMapping(a2ofR1, objs.get(4));
				m.addMapping(a3ofR1, objs.get(5));
	...
	private boolean applyRule2(
		...
		for (int i=0; i<matchVecForR.size(); i++) {
			Vector<GraphObject> objs = matchVecForR.get(i);
			try {							
				m.addMapping(n1ofR2, objs.get(0));
				m.addMapping(n2ofR2, objs.get(1));
				m.addMapping(n3ofR2, objs.get(2));
				m.addMapping(a1ofR2, objs.get(3));
				m.addMapping(a2ofR2, objs.get(4));
				m.addMapping(a3ofR2, objs.get(5));
			} catch (BadMappingException ex) {
...
/*	Gabi's code
	void applyTriangles1(SierpinskiImpl s, Vertex root, Vertex currentVertex){
        ...
        addtr.setVertex1(leftVertex);
        addtr.setVertex2(rightVertex);
        //if (addtr.canExecute()) System.out.println("Regel 1 kann 
//angewandt werden.");
        //else System.out.println("Regel 1 kann nicht angewandt werden.");
        addtr.execute();
   ...
   void applyTriangles2(SierpinskiImpl s, Vertex root, Vertex 
currentVertex){
        ...
        if (currentVertex.getConn().size() > 0) {
            ...
            Vertex rightVertex = currentVertex.getRight();
            addtr2.setVertex1(leftVertex);
            addtr2.setVertex2(rightVertex);
            //if(leftVertex.getConn().get(0) == rightVertex)
            //     System.out.println("ist gleich");
            //if (addtr2.canExecute()) System.out.println("Regel 2 kann 
//angewandt werden.");
            //else System.out.println("Regel 2 kann nicht angewandt 
//werden.");
            addtr2.execute();
