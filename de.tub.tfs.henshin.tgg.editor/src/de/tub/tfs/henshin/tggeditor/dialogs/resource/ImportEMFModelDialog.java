...
public class ImportEMFModelDialog extends RegisteredPackageDialog implements
	...
	/** Whether it is Dialog for Source, Target oder Correspondence Dialog */
	 ...
	 * @param arg0 the arg0
	 */
	public ImportEMFModelDialog(Shell arg0, String importModel) {
		super(arg0);
		super.setTitle(EcoreEditorPlugin.INSTANCE.getString("_UI_PackageSelection_label")
				+" for "+importModel+" Model");
	...
	protected void computeResult() {
		...
		if (this.imported != null) {
			...
			for (Object pkg : getSelectedElements()) {
				EPackage e = EPackage.Registry.INSTANCE.getEPackage((String) pkg);
				imported.add(e);
			}
		}
	...
	public void widgetSelected(SelectionEvent e) {
		ResourcesDialog sysDiag = new ResourcesDialog(getShell(), null,
				"Import EMF "+this.importModel+" Model", "Select a resource with an EMF model.",
				SWT.OPEN);
		...
		if (sysDiag.getReturnCode() == OK) {
			...
			if (!src.getContents().isEmpty() && src.getContents().get(0) instanceof EPackage )
				src.setURI(URI.createURI(((EPackage)src.getContents().get(0)).getNsURI()));
			try {
