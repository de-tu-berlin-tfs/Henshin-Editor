...
public class TGGClipboardSupport  extends AbstractClipboardSupport implements IClipboardSupport2 {
	...
	private final class LoadingEMFResourceExtension extends LoadingEMFResource {
		...
		protected void doUnload() {
			CompoundCommand nodeCommands = new CompoundCommand();
			CompoundCommand edgeCommands = new CompoundCommand();
			CompoundCommand attrCommands = new CompoundCommand();
			
			for (Object pasted : op.getPastedElementSet()) {
				if (pasted instanceof EObject){
					if (((EObject) pasted).eClass().getEPackage().equals(TggPackage.eINSTANCE)) {
						EStructuralFeature marker = ((EObject) pasted).eClass().getEStructuralFeature("markerType");
						if (marker != null){
							String markerString = (String) ((EObject) pasted).eGet(marker);
							if (!RuleUtil.NEW.equals(markerString) && !RuleUtil.TR_UNSPECIFIED.equals(markerString)){
								EObject eContainer = ((EObject) pasted).eContainer();
								if (eContainer instanceof Graph){
									Graph containingGraph = (Graph) eContainer;
									if (containingGraph.isLhs() || containingGraph.isRhs()){
										
										if (pasted instanceof Node){
											((EObject) pasted).eSet(marker, RuleUtil.NEW);
											MarkCommand cmd = new MarkCommand((Node) pasted);
											
											nodeCommands.add(cmd);
										} else if (pasted instanceof Edge){
											((EObject) pasted).eSet(marker, RuleUtil.NEW);
											MarkEdgeCommand cmd = new MarkEdgeCommand((Edge) pasted);
											
											edgeCommands.add(cmd);
										} else if (pasted instanceof Attribute){
											((EObject) pasted).eSet(marker, RuleUtil.NEW);
											MarkAttributeCommand cmd = new MarkAttributeCommand((Attribute) pasted);
											
											attrCommands.add(cmd);
										}
									}
			...
			nodeCommands.execute();
			edgeCommands.execute();
			attrCommands.execute();
			super.doUnload();
		}
	}
