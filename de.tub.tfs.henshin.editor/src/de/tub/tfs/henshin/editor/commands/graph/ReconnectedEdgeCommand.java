...
public class ReconnectedEdgeCommand extends Command {
	...
	public boolean canExecute() {
		eReferences = EdgeReferences.getSourceToTargetFreeReferences(source,
				target);
		return (eReferences.size() > 0
				|| (edge.getSource() == source && edge.getTarget() == target)) && !HenshinLayoutUtil.INSTANCE.hasOriginInKernelRule(source) && !HenshinLayoutUtil.INSTANCE.hasOriginInKernelRule(target);
	}
	...
	public void execute() {
		...
		List<EReference> references = EdgeReferences
