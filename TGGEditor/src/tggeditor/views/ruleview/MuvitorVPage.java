...
import tggeditor.actions.create.SelectAllInMultiViewerAction;
 ...
 * @author Tony Modica
 */
public abstract class MuvitorVPage extends Page implements IAdaptable, CommandStackListener,
		IGraphicalViewerProvider, ISelectionListener {
	...
	final public class MultiViewerPageViewer extends ScrollingGraphicalViewer {
		 ...
		 */
		public final MuvitorVPage getHostPage() {
			return MuvitorVPage.this;
		}
	...
	private final Adapter adapter = new AdapterImpl() {
		...
		public void notifyChanged(final Notification msg) {
			MuvitorVPage.this.notifyChanged(msg);
	 ...
	 */
	public MuvitorVPage(final MuvitorPageBookView view) {
		this.view = view;
	}
	 ...
	 * {@link #setCurrentViewer(MuvitorVPage.MultiViewerPageViewer)} .
	...
	public final void createControl(final Composite parent) {
		...
		flyoutPaletteComposite = new FlyoutPaletteComposite(parent, SWT.NONE, getSite().getPage(),
	...
	public final GraphicalViewer getViewer(final EObject forModel) {
		for (final MultiViewerPageViewer viewer : viewers) {
			if (viewer.getContents() != null && viewer.getContents().getModel() == forModel) {
				return viewer;
			}
		}
		return null;
	...
	public final EObject getViewersContents(final int viewerPosition) {
		Assert.isTrue(viewerPosition < getNumberOfViewers(),
				"AbstractMultiViewerPage tried to retrieve contents for viewer on position"
						+ viewerPosition + " but has only " + getNumberOfViewers() + "viewers!");
	 ...
	 * @see #getViewerContents()
	 */
	public final void setViewersContents(final int viewerPosition, final EObject model) {
		Assert.isTrue(viewerPosition < getNumberOfViewers(),
	 ...
	 *            the position (starting with 0) of the viewer
	 */
	public final void setViewerVisibility(final int viewerPosition, final boolean visible) {
		Assert.isTrue(viewerPosition < getNumberOfViewers(),
	 ...
	 */
	protected final Composite createGraphicalViewerComposite() {
...
//		viewerComposite.setLayout(new FillLayout());
		...
		for (int i = 0; i < contentsSize; i++) {
			...
			final RulerComposite rulerComp = new RulerComposite(viewerComposite, SWT.NONE);
			...
			// parent
			final KeyHandler graphicalViewerKeyHandler = new GraphicalViewerKeyHandler(viewer);
			...
			if (isContextMenuRegistered()) {
				getEditor().getEditorSite().registerContextMenu(cmp, getCurrentViewer());
			}
			...
			final ZoomManager zoomManager = (ZoomManager) viewer.getProperty(ZoomManager.class
					.toString());
			...
			viewer.setProperty(RulerProvider.PROPERTY_HORIZONTAL_RULER, new MuvitorRulerProvider());
			viewer.setProperty(RulerProvider.PROPERTY_VERTICAL_RULER, new MuvitorRulerProvider());
			viewer.setProperty(RulerProvider.PROPERTY_RULER_VISIBILITY, Boolean.FALSE);
			viewer.setProperty(SnapToGrid.PROPERTY_GRID_SPACING, new Dimension(20, 20));
		}
	...
	private final void installThumbnailInPalette() {
		...
		final FigureCanvas figureCanvas = (FigureCanvas) getEditDomain().getPaletteViewer()
				.getControl();
		
		// install a SashForm on the parent of the FigureCanvas
		final SashForm sashForm = new SashForm(figureCanvas.getParent(), SWT.VERTICAL);
	...
	protected final KeyHandler getSharedKeyHandler() {
		if (keyHandler == null) {
			...
			// this may be null
			keyHandler.setParent((KeyHandler) getEditor().getAdapter(KeyHandler.class));
		}
		return keyHandler;
	}
	...
	protected final IAction registerActionAsHandler(final IAction action) {
		...
		getSite().getActionBars().setGlobalActionHandler(id, registerAction(action));
		return action;
	}
	...
	protected final IAction registerSharedAction(final String id) {
		...
		Assert.isNotNull(action, "The editor did not deliver an action with the id '" + id
				+ "' for the page!");
	...
	protected final void setCurrentViewer(final MultiViewerPageViewer viewer) {
		Assert.isNotNull(viewer, "AbstractMultiViewerPage can not be set to viewer 'null'!");
